...
public abstract class BenchmarkCharsetRunner<TestResult, BenchmarkResult> {
  ...
  public record BenchmarkConfig(
      ...
      int minCharsetLength,
      int maxCharsetLength,
      int testNumber) {
    ...
    private final static int DEFAULT_MIN_CHARSET_LENGTH = 2;
    private final static int DEFAULT_MAX_CHARSET_LENGTH = 52;
    ...
    public BenchmarkConfig() {
      this(
          ...
          DEFAULT_MIN_CHARSET_LENGTH,
          DEFAULT_MAX_CHARSET_LENGTH,
  ...
  public BenchmarkCharsetRunner(BenchmarkConfig config) {
    this.config = new GeneralConfig(config);
  }

  public BenchmarkCharsetRunner(GeneralConfig config) {
    this.config = config;
  }

  public BenchmarkCharsetRunner() {
  ...
  public void benchmarkRandomTest() throws Exception {
    ...
    for (int rsl = config.benchConfig.maxRandomStringLength(); rsl >= config.benchConfig
      ...
      for (int msl = config.benchConfig.minMaxSequenceLength(); msl <= config.benchConfig
        ...
        for (int cl = config.benchConfig.maxCharsetLength(); cl >= config.benchConfig
            .minCharsetLength(); cl = (int) Math.max(1, cl * 0.8)) {
          allParams.add(new InputParam(rsl, msl, cl));
        }
      }
    }
      ...
      futures.add(executor.submit(() -> {
        ...
        for (int i = 0; i < config.benchConfig.testNumber(); i++) {
          RandomInputHelper.InputLine input = RandomInputHelper.generateLine(
              ...
              RandomInputHelper.generateCharset(inputParam.charsetLength()),
              localRandom);
